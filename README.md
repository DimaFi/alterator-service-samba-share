...
Этот скрипт предназначен для удобного и надёжного управления общими папками (шарами) Samba. Он позволяет ~~создавать~~, ~~удалять~~, запускать, останавливать и просматривать Samba-шары, а также управлять соответствующими службами Samba.
...
Обязательные параметры зависят от действия (`mode`):
...
## Новые возможности, добавленные сегодня

- Перенос основного скрипта в `/usr/bin` для удобства и стандартизации запуска
- Введение backend-файла с функциями в `/usr/share/alterator-service-samba/backend.sh`
- Создание systemd unit-файла `/etc/systemd/system/samba_share.service` для автоматизации запуска
- Внедрение полноценного логирования всех операций с подробной информацией в `/var/log/samba-shares.log`
- Добавлена поддержка команд:
  - `deploy` — развёртывание и активация служб Samba
  - `undeploy` — остановка и отключение служб Samba
  - `start` и `stop` — запуск и остановка службы `smb.service`
  - `status` — вывод статуса служб и текущих шар с кодами выхода для автоматической обработки
- Реализован парсер вывода `testparm -s` для формирования актуального списка Samba-шар в формате JSON, включающий имя, путь, тип доступа и пользователя (если известен)
- Включена проверка прав доступа и корректности параметров перед выполнением действий
- Логика блокировки и разблокировки процесса с гарантией освобождения ресурса при выходе или ошибках
...
```bash
echo '{"mode":"create","share_name":"data","share_path":"/srv/data","access_user":"alice","access_type":"rw"}' | /usr/bin/samba_share.sh
```
